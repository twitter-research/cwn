...
def test_get_all_chain_params_with_max_dim_one_and_no_top_features():
    house_complex = get_house_complex()

    params = house_complex.get_all_chain_params(max_dim=1, include_top_features=False)
    assert len(params) == 2

    v_chain_params, e_chain_params = params
    ...
    assert e_chain_params.up_index is not None
    assert e_chain_params.up_index.size(1) == 6
