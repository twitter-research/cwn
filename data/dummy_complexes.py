...
def get_testing_complex_list():
    return [get_square_dot_complex(), get_pyramid_complex()]
...
def get_kite_complex():
    ...
    e_shared_faces = torch.tensor([1, 1, 1, 1, 1, 1, 0, 0, 2, 2, 2, 2, 2, 2, 3, 3, 3, 3, 3, 3], dtype=torch.long)
    ...
    e_x = torch.tensor([[1], [2], [3], [4], [5], [6]], dtype=torch.float)
    ye = torch.tensor([1, 1, 1, 1, 1, 1], dtype=torch.long)
    e_chain = Chain(dim=1, x=e_x, lower_index=e_down_index, shared_faces=e_shared_faces, upper_index=e_up_index, shared_cofaces=e_shared_cofaces, y=ye)
...
def get_pyramid_complex():
 ...
 3-----0-----3
  ...
  """
    ...
    e_x = torch.tensor([[1], [2], [3], [4], [5], [6]], dtype=torch.float)
    ye = torch.tensor([1, 1, 1, 1, 1, 1], dtype=torch.long)
    e_chain = Chain(dim=1, x=e_x, lower_index=e_down_index, upper_index=e_up_index, shared_faces=e_shared_faces, shared_cofaces=e_shared_cofaces, y=ye)
    ...
    t_chain = Chain(dim=2, x=t_x, lower_index=t_down_index, upper_index=t_up_index, shared_faces=t_shared_faces, shared_cofaces=t_shared_cofaces, y=yt)
    
    p_x = torch.tensor([[1]], dtype=torch.float)
    yp = torch.tensor([3], dtype=torch.long)
    p_chain = Chain(dim=3, x=p_x, y=yp)
