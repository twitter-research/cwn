...
def convert_to_graph(complex):
    ...
    assert complex.chains[0].num_cells > 0
    ...
    if chain.upper_index is None:
        ...
        if 1 in complex.chains and complex.chains[1].x is not None and chain.shared_coboundaries is not None:
            edge_attr = torch.index_select(complex.chains[1].x, 0, chain.shared_coboundaries)
...
def get_house_complex():
    ...
    v_chain = Chain(dim=0, x=v_x, upper_index=v_up_index, shared_coboundaries=v_shared_coboundaries, y=yv)
    ...
    t_chain = Chain(dim=2, x=t_x, y=yt, boundary_index=t_boundary_index)
...
def get_bridged_complex():
    ...
    t_chain = Chain(dim=2, x=t_x, y=yt, boundary_index=t_boundary_index, lower_index=t_down_index, shared_boundaries=t_shared_boundaries)
...
def get_square_complex():
    ...
    e_chain = Chain(dim=1, x=e_x, lower_index=e_down_index, shared_boundaries=e_shared_boundaries, y=ye,
...
def get_kite_complex():
    ...
    e_x = torch.tensor([[1], [2], [3], [4], [5], [6]], dtype=torch.float)
    ye = torch.tensor([1, 1, 1, 1, 1, 1], dtype=torch.long)
    e_chain = Chain(dim=1, x=e_x, lower_index=e_down_index, shared_boundaries=e_shared_boundaries,
    ...
    t_chain = Chain(dim=2, x=t_x, lower_index=t_down_index, shared_boundaries=t_shared_boundaries, y=yt,
...
def get_pyramid_complex():
 ...
 3-----0-----3
  ...
  """
    ...
    p_chain = Chain(dim=3, x=p_x, y=yp, boundary_index=p_boundary_index)
...
def get_filled_square_complex():
    ...
    c_chain = Chain(dim=2, x=c_x, y=yc, boundary_index=c_boundary_index)
...
def get_molecular_complex():
    ...
    c_chain = Chain(dim=2, x=c_x, y=yc, boundary_index=c_boundary_index, lower_index=c_down_index,
