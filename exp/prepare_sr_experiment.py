...
def prepare(family, jobs, max_ring_size, permute, seed):
    root = os.path.join(ROOT_DIR, 'datasets')
    raw_dir = os.path.join(root, 'SR_graphs', 'raw')
    _ = load_dataset(family, max_dim=2, max_ring_size=max_ring_size, n_jobs=jobs)
    if permute:
        ...
        _ = load_dataset(f'{family}p{seed}', max_dim=2, max_ring_size=max_ring_size, n_jobs=jobs)
    return
...
if __name__ == "__main__":
    ...
    for family in __families__:
        ...
        prepare(family, jobs, max_ring_size, permute=='y', 43)
