...
python -m exp.run_exp --dataset ZINC --train_eval_period 10 --epochs 300 --batch_size 64   \
  ...
  --init_method sum --jump_mode cat --lr 0.003 --model embed_sparse_sin --nonlinearity relu \
...
python -m exp.run_mol_exp --dataset ZINC --seeds 10 --train_eval_period 10 \
  ...
  --max_dim 2 --final_readout sum --init_method sum --lr 0.001 --model embed_sparse_sin \
...
python -m exp.run_mol_exp --seeds 20 --folds 5 --exp_name csl-sin \
