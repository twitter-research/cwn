class Chain(object):
    ...
    '''
	def __init__(self, order, x=None, upper_index=None, lower_index=None, y=None):
        '''
            ...
            - `upper_index`: upper adjacency, matrix, shape [order, num_upper_connections]; may not be available, e.g. when `order` is the top level order of a complex
            - `lower_index`: lower adjacency, matrix, shape [order, num_lower_connections]; may not be available, e.g. when `order` is the top level order of a complex
